...
class OpenIDConnectClient
{
    ...
    {
        $this->responseTypes = array_merge(
            $this->responseTypes,
    ...
    {
        $sign_out_endpoint = $this->getProviderConfigValue(
        ...
        $sign_out_endpoint .=
            (strpos($sign_out_endpoint, "?") === false ? "?" : "&") .
    ...
    public function verifyLogoutToken(): bool
    {
        ...
        throw new OpenIDConnectClientException(
            "Back-channel logout: There was no logout_token in the request"
        );
    }
    ...
    {
        $this->registrationParams = array_merge(
            $this->registrationParams,
            $param
        );
    }

    public function setTokenEndpointAuthMethodsSupported(
    ...
    protected function getProviderConfigValue(string $param, $default = null)
    {
        ...
        if (!isset($this->providerConfig[$param])) {
            $this->providerConfig[$param] = $this->getWellKnownConfigValue(
                $param,
                $default
            );
        }
    ...
    public function getRedirectURL(): string
    {
        ...
        if (
            $this->httpUpgradeInsecureRequests &&
        ...
        $explodedRequestUri = isset($_SERVER["REQUEST_URI"])
    ...
    {
        $auth_endpoint = $this->getProviderConfigValue(
            ...
            if (!empty($this->pkceAlgs[$codeChallengeMethod])) {
                $codeChallenge = rtrim(
                    strtr(
                        base64_encode(
                            hash(
                                $this->pkceAlgs[$codeChallengeMethod],
                                $codeVerifier,
                                true
                            )
                        ),
        ...
        $auth_endpoint .=
            ...
            http_build_query($auth_params, "", "&", $this->encType);
    ...
    public function requestClientCredentialsToken()
    {
        ...
        return json_decode(
            $this->fetchURL($token_endpoint, $post_params, $headers),
            false
        );
    }
    ...
    public function requestResourceOwnerToken(bool $bClientAuth = false)
    {
        ...
        if ($bClientAuth) {
            $token_endpoint_auth_methods_supported = $this->getProviderConfigValue(
                "token_endpoint_auth_methods_supported",
                ["client_secret_basic"]
            );
            if (
                $this->supportsAuthMethod(
                    "client_secret_basic",
                    $token_endpoint_auth_methods_supported
                )
            ) {
    ...
    protected function requestTokens(string $code, array $headers = [])
    {
        ...
        if (
            $this->privateKeyJwtGenerator !== null &&
            $this->supportsAuthMethod(
                "private_key_jwt",
                $token_endpoint_auth_methods_supported
            )
        ) {
            $token_params["client_assertion_type"] =
                "urn:ietf:params:oauth:client-assertion-type:jwt-bearer";
            $token_params[
                "client_assertion"
            ] = $this->privateKeyJwtGenerator->__invoke($token_endpoint);
        }
        ...
        if (
            ...
            $client_assertion_type = $this->getProviderConfigValue(
            ...
            if (isset($this->providerConfig["client_assertion"])) {
                ...
                $client_assertion = $this->getJWTClientAssertion(
                    $this->getProviderConfigValue("token_endpoint")
                );
            }
        ...
        $this->tokenResponse = json_decode(
            $this->fetchURL($token_endpoint, $token_params, $headers),
            false
        );
    ...
    public function refreshToken(string $refresh_token)
    {
        ...
        if (
            ...
            $token_params["subject_token_type"] =
                "urn:ietf:params:oauth:token-type:refresh_token";
            $token_params["requested_token_type"] =
    ...
    private function verifyRSAJWTSignature(
        ...
        $key = RSA::load([
            "publicExponent" => new BigInteger(
                base64_decode(b64url2b64($key->e)),
                256
            ),
            "modulus" => new BigInteger(
                base64_decode(b64url2b64($key->n)),
                256
            ),
            "isPublicKey" => true,
        ])->withHash($hashType);
    ...
    public function verifyJWTSignature(string $jwt): bool
    {
        ...
        switch ($header->alg) {
            ...
            case "RS512":
                ...
                $signatureType =
                    $header->alg === "PS256" || $header->alg === "PS512"
                        ? "PSS"
                        : "";
                if (isset($header->jwk)) {
                    $jwk = $header->jwk;
            ...
            default:
                throw new OpenIDConnectClientException(
                    "No support for signature type: " . $header->alg
                );
        }
        return $verified;
    }
    ...
    protected function verifyJWTClaims(
        ...
        if (isset($claims->at_hash, $accessToken)) {
            if (
                isset($this->getIdTokenHeader()->alg) &&
                $this->getIdTokenHeader()->alg !== "none"
            ) {
            ...
            $len = ((int) $bit) / 16;
            $expected_at_hash = $this->urlEncode(
                substr(hash("sha" . $bit, $accessToken, true), 0, $len)
            );
        }
        $auds = $claims->aud;
        $auds = is_array($auds) ? $auds : [$auds];
        return $this->validateIssuer($claims->iss) &&
            ...
            (!isset($claims->nbf) ||
                (is_int($claims->nbf) &&
                    $claims->nbf <= time() + $this->leeway)) &&
            (!isset($claims->at_hash) ||
                !isset($accessToken) ||
                $claims->at_hash === $expected_at_hash);
    }
    ...
    {
        $user_info_endpoint = $this->getProviderConfigValue(
            "userinfo_endpoint"
        );
     ...
     * defaults to true
     */
    public function setHttpUpgradeInsecureRequests(
        bool $httpUpgradeInsecureRequests
    ) {
    ...
    {
        $registration_endpoint = $this->getProviderConfigValue(
    ...
    public function introspectToken(
        ...
        $introspection_endpoint = $this->getProviderConfigValue(
        ...
        $headers = [
            "Authorization: Basic " .
            base64_encode(
                urlencode($clientId) . ":" . urlencode($clientSecret)
            ),
            "Accept: application/json",
        ];
        ...
        return json_decode(
            $this->fetchURL($introspection_endpoint, $post_params, $headers),
            false
        );
    }
    ...
    public function revokeToken(
        ...
        $revocation_endpoint = $this->getProviderConfigValue(
            "revocation_endpoint"
        );
        ...
        return json_decode(
            $this->fetchURL($revocation_endpoint, $post_params, $headers),
            false
        );
    }
    ...
    {
        throw new OpenIDConnectClientException(
            "Self signed JWK header is not valid"
        );
    }
    ...
    {
        throw new OpenIDConnectClientException(
            "JWE response is not supported, please extend the class and implement this method"
        );
    }
    ...
    public function supportsAuthMethod(
        string $auth_method,
        array $token_endpoint_auth_methods_supported
    ): bool {
